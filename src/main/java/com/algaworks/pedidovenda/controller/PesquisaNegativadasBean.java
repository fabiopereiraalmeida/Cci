...
public class PesquisaNegativadasBean implements Serializable {
	...
	public void excluir() {
		try {
			...
			FacesUtil.addInfoMessage("Negativada " + negativadaSelecionado.getPessoa() + " excluída com sucesso.");
	...
	public void baixa() {
		...
		try {
			...
			FacesUtil.addInfoMessage(
					"A negativada de " + negativadaSelecionado.getPessoa().getNome() + " foi dado baixa com sucesso.");
		} catch (Exception e) {
	...
	public void emitirCarta() {
		try {
			...
			ExecutorRelatorio executor = new ExecutorRelatorio("/relatorios/comunicado.jasper", response, parametros,
					"Comunicado.pdf");
